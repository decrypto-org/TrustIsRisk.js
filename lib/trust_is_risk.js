...
class TrustIsRisk {
  ...
  getIndirectTrust(origin         , dest         ) {
    return this.db.getTrustAmount(origin, dest);
  }

  getDirectTrust(origin         , dest         ) {
  ...
  async createTrustIncreasingMTX(origin      , dest      , outpoint                 ,
      trustAmount         , fee          )
                           {
  ...
  createTrustDecreasingMTXs(origin      , dest      , trustDecreaseAmount         , payee          ,
      steal           , fee          )               {
  ...
  getTrustDecreasingMTX(directTrust              , decreaseAmount         , payee          ,
      signingKeyRing                , fee          ) {
  ...
  // the recipient.
  searchForDirectTrustOutputs(tx           , origin         , recipient          )                 {
  ...
  isChangeOutput(output               , origin         )           {
    return (output.getType() === "pubkeyhash")
            && (output.getAddress().toBase58() === origin);
  }

  parseOutputAsDirectTrust(tx           , outputIndex         , origin         )
                             {
    var txHash = tx.hash().toString("hex");
